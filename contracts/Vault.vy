...
# Cherry Vault
#
# The main difference between the Cherry vault and Yearn is that some Cherry debt is illiquid and cannot be withdrawn.
# The user is given an option to withdraw when debt is nonzero but at the cost of a haircut proportional to the (illiquid amount / total assets)
# Illiquid debt should be tracked separately, since the portion of debt that is just kept as ETH/WETH can be withdrawn.
...
struct StrategyParams:
    ...
    maxIlliquidDebt: uint256  # Maximum illiquid debt
    illiquidDebt:  uint256 # Current amount of illiquid debt
    illiquidDebtRatio: uint256 # Portion of illiquid debt the strategy has
...
event StrategyAdded:
    ...
    debtRatio: uint256  # Maximum borrow amount (in BPS of total assets)
    illiquidDebtRatio: uint256  # Max portion of illiquid debt the strategy can have
    minDebtPerHarvest: uint256  # Lower limit on the increase of debt since last harvest
    maxDebtPerHarvest: uint256  # Upper limit on the increase of debt since last harvest
    performanceFee: uint256  # Strategist's fee (basis points)
...
illiquidDebtRatio: public(uint256)  # Ratio of illiquid debt across all strategies
totalDebt: public(uint256)  # Amount of tokens that all strategies have borrowed
totalIlliquidDebt: public(uint256) # Total amount of illiquid debt across all strategies
...
def initialize(
    ...
    self.lockedProfitDegradation = convert((28 * DEGRADATION_COEFFICIENT) * 46 / 10 ** 6 , uint256) # 7 days in blocks
    # EIP-712
...
def addStrategy(
    strategy: address,
    maxIlliquidDebt : uint256, 
    illiquidDebt : uint256, 
    illiquidDebtRatio : uint256,
    debtRatio: uint256,
    ...
    self.strategies[strategy] = StrategyParams({
        ...
        maxIlliquidDebt : maxIlliquidDebt, 
        illiquidDebt : illiquidDebt, 
        illiquidDebtRatio : illiquidDebtRatio,
        debtRatio: debtRatio,
    ...
    log StrategyAdded(strategy, debtRatio, illiquidDebtRatio, minDebtPerHarvest, maxDebtPerHarvest, performanceFee)
...
def migrateStrategy(oldVersion: address, newVersion: address):
    ...
    self.strategies[newVersion] = StrategyParams({
        ...
        maxIlliquidDebt : strategy.maxIlliquidDebt, 
        illiquidDebt : strategy.illiquidDebt, 
        illiquidDebtRatio : strategy.illiquidDebtRatio,
